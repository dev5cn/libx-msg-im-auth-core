...
void XmsgNe::estab()
{
	...
	req->set_neg(X_MSG_IM_AUTH);
	req->set_cgt(XmsgImAuthCfg::instance()->cfgPb->cgt());
	req->set_salt(Crypto::gen0aAkey256());
	req->set_sign(Crypto::sha256ToHexStrLowerCase(XmsgImAuthCfg::instance()->cfgPb->cgt() + req->salt() + this->pwd));
